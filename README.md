...
- **386 爻辞全收录**：64卦×6爻 + 乾用九/坤用六，每条含原文、白话、英译；变爻时逐条展示动爻爻辞（占断精髓），爻名已程序化校验与卦形阴阳完全一致
- **分享图卡**：结果页一键分享古风卦象卡片（3x高清渲染，语言跟随App，附文字版）
- **每日提醒**：设置中开启自选时间的本地通知，提醒每日一占
- **会员订阅**：解锁每卦事业/爱情/财运/健康四维深度详批（每维三段式：卦象分析→具体建议→提醒宜忌，中文均 150+ 字），StoreKit 2 月度/年度
- **福币系统**：钱包与签到记录存 iCloud 键值存储——重装 App 不丢，同一 Apple ID 的多台设备自动同步；每日签到 +1 福币（会员 +2）；签到日历以红圈标记已签到日，可翻看历史月份；每日免费一卦，重算不限次数但价格阶梯递增（10→30→300→3000…每次×10，践行「再三渎，渎则不告」），跨设备同步计数，警告随次数递进；福币内购四档：$0.99=30币、$4.99=180币（省16%）、$19.99=800币（省24%）、$49.99=2500币（省39%），优惠比例由实际单价动态计算并在购买页显示
- **占卜历史**：每日全部卦象（含重算）逐条存档（近1000条），同日多卦以精确时间区分，随时回看验证
...
- **起卦仪式**：写下所问后进入暗色"静室"——磬声起，随圆环三次呼吸（震动随息），闭目默念所问，长按朱印「誠」承诺（不诚不占），方入掷币；可跳过、设置可关；问候语随晨午夜时辰变化
- **揭卦过场**：六爻掷毕，墨色屏上卦象如墨迹晕开（模糊渐聚），短磬一响、卦名吉凶浮现，方落入结果页；轻点可跳过
...
工程含两个 target：主 App 与 `DailyHexagramWidget`（Widget Extension），通过 App Group `group.com.octlex.dailyhexagram` 共享数据。
